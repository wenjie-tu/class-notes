...
Compute 95% CIs for the true probability of response in the Secukinumab and in the placebo groups:
...
Wilson method is used to compute the 95% CIs for the true probability of response in Secukinumab and in the placebo groups. Wilson confidence interval is recommended in this setting since it does not suffer from problems of overshoot and zero-width intervals that afflict the normal interval and it can also be safely used with small samples and skewed observations. To show the difference among CIs computed from other methods, the proportion test and the exact binomial test are conducted for comparison.
...
Plot prior $\textrm{Beta}(0.5,1)$ for Secukinumab group and prior $\textrm{Beta}(11,32)$ for placebo group:
...
Implementation with a function in \texttt{R}:
