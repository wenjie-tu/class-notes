...
$$
t=\frac{\hat{\theta}_1-\theta_1}{\frac{\textrm{SE}(\hat{\theta})}{\sqrt{n}}}=
\frac{0.07}{\frac{0.4}{\sqrt{30}}}=0.959
$$

By looking up $t$-Distribution table with 29 ($30-1=29$) degrees of freedom. we find that 0.959 falls between 0.854 and 1.055. The corresponding $p$-value for a two-sided test is between 0.40 and 0.30. Alternatively, we can use the command \texttt{2$*$pt(0.959, df=30-1, lower.tail=FALSE)} in R console to get the corresponding $p$-value (0.345). The $p$-value is **NOT** less than the $\alpha=0.05$ level, we cannot reject the null hypothesis. Hence, we do not have sufficient evidence to say that the coefficient $\theta_1$ is statistically different from zero.
